...
import static org.httpkit.HttpUtils.bodyBuffer;
...
import static org.httpkit.server.WSDecoder.OPCODE_BINARY;
import static org.httpkit.server.WSDecoder.OPCODE_CLOSE;
import static org.httpkit.server.WSDecoder.OPCODE_TEXT;
...
public class AsyncChannel {
    ...
    public AsyncChannel(SelectionKey key, IOWorker server) {
        this.key = key;
        this.server = server;
    }
    ...
    public void reset(HttpRequest request) {
        ...
        unsafe.putOrderedObject(this, closeHandlerOffset, null); // lazySet to
        // null
        unsafe.putOrderedObject(this, receiveHandlerOffset, null); // lazySet to
        // null
    }
    ...
    private void firstWrite(Object data, boolean close) throws IOException {
        ...
        if (close) { // normal response, Content-Length. Every http client understand it
            ...
            if (body == null) {
                ...
                buffers = new ByteBuffer[]{bb[0], // header
    ...
    private void writeChunk(Object body, boolean close) throws IOException {
        ...
        if (body != null) { // null is ignored
            ...
            if (t.hasRemaining()) {
                ByteBuffer[] buffers = new ByteBuffer[]{chunkSize(t.remaining()), t, // actual
                        // data
                        ByteBuffer.wrap(newLineBytes) // terminating CRLF
                        // sequence
                };
